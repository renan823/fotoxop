import type { Point, Scale } from "../types";
import { curveCatmullRom } from "../utils";

/*
Transformação por curva de intensidade.

T(z) = lookup[z]

Onde 'lookup' é uma table de valores criada
a partir da interpolação dos pontos
da curva de intesidade definida.
*/
export class ToneCurveTransform {
    public static execute(image: ImageData, points: Point[]): ImageData {
        const result = new ImageData(image.width, image.height);
        result.data.set(image.data);

        const lookup = ToneCurveManager.interpolate(points);

        for (let i = 0; i < result.data.length; i += 4) {
            result.data[i] = lookup[result.data[i]];
            result.data[i + 1] = lookup[result.data[i + 1]];
            result.data[i + 2] = lookup[result.data[i + 2]];
        }

        return result;
    }
}

/*
Classe que agrupa rotinas usadas na manipulação
da curva de intesidade.
*/
export class ToneCurveManager {
    /*
    Gera os pontos entre o começo e o fim da escala.
    Depende do máximo de pontos especificado.
    */
    public static init(n: number): Point[] {
        const A: Point = { id: 0, x: 5, y: 5 };
        const B: Point = { id: n - 1, x: 250, y: 250 };

        const points: Point[] = [];

        for (let i = 0; i < n; i++) {
            const t = i / (n - 1);

            points.push({
                id: i,
                x: A.x + t * (B.x - A.x),
                y: A.y + t * (B.y - A.y),
            });
        }

        return points;
    }

    /*
    Função que interpola os pontos gerando a taebla de correspondencia.
    Usando a interpolação, cada valor de x é mapeado pra um de y
    dentro do domínio 0-255.

    Usa a função de interpolação Catmull-Rom pra gerar a tabela.
    */
    public static interpolate(points: Point[]): Uint8ClampedArray {
        const lookup = new Uint8ClampedArray(256);

        points = points.map((p) => ({
            id: p.id,
            x: Math.round(p.x),
            y: p.y,
        }));

        for (let x = 0; x < 256; x++) {
            let i = 0;
            while (i < points.length - 1 && x > points[i + 1].x) {
                i++;
            }

            const p0 = points[Math.max(0, i - 1)];
            const p1 = points[i];
            const p2 = points[Math.min(points.length - 1, i + 1)];
            const p3 = points[Math.min(points.length - 1, i + 2)];

            const dx = p2.x - p1.x;
            if (dx === 0) {
                lookup[x] = p1.y;
                continue;
            }

            const t = Math.max(0, Math.min(1, (x - p1.x) / dx));
            const y = curveCatmullRom(p0.y, p1.y, p2.y, p3.y, t);

            lookup[x] = Math.max(0, Math.min(255, Math.round(y)));
        }

        return lookup;
    }

    /*
    Limita a movimentação de um ponto "esbarrando"
    nos vizinhos.
    Um ponto não pode ultrapassar seus vizinhos.
    */
    public static limitByNeighbors(
        points: Point[],
        index: number,
        p: Point
    ): Point {
        const tolerance = 1;

        const point = { ...p };
        const prev = points[index - 1];
        const next = points[index + 1];

        if (prev) {
            point.x = Math.max(point.x, prev.x + tolerance);
        }

        if (next) {
            point.x = Math.min(point.x, next.x - tolerance);
        }

        return point;
    }

    /*
    Função que aplica movimentação do ponto que foi
    arrastado, puxando um pouco seus vizinhos.
    Limita a movimentação pelo tamanho da tela e
    pelos vizinhso diretos.

    Pra lidar corretamente, é necessário converter entre
    a escala de cores e a escala real da tela.

    A parte de "puxar" vizinhos próximos foi feita
    com ajuda de IA.
    */
    public static move(
        points: Point[],
        d: Point,
        px: number,
        py: number,
        xScale: Scale,
        yScale: Scale
    ): Point[] {
        // Pegar ponto atual
        const index = points.findIndex((p) => p.id === d.id);
        if (index === -1) {
            return points;
        }

        if (px === undefined || py === undefined) {
            return points;
        }

        let point = {
            ...points[index],
            x: xScale.invert(px),
            y: yScale.invert(py),
        };

        point.x = Math.max(0, Math.min(point.x, 255));
        point.y = Math.max(0, Math.min(point.y, 255));

        // Limitar até vizinhos
        point = this.limitByNeighbors(points, index, point);

        // Puxar vizinhos um pouco pra suavizar
        const dy = point.y - points[index].y;
        const influence = 1;

        const updated = points.map((p, i) => {
            if (i === index) {
                return point;
            }

            const dist = Math.abs(i - index);
            const weight = Math.exp(-dist);

            const py = p.y + dy * influence * weight;

            return {
                ...p,
                y: Math.max(0, Math.min(py, 255)),
            };
        });

        return updated;
    }
}
